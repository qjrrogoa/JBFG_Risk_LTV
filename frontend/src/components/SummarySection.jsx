    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-rose-50 border border-rose-100 rounded-2xl p-4">
          ...
          <div className="text-4xl font-black text-rose-600 leading-none tabular-nums mt-2">{redCnt}</div>
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4">
          ...
          <div className="text-4xl font-black text-amber-600 leading-none tabular-nums mt-2">{yellowCnt}</div>
          <div className="text-xs text-slate-400 mt-1.5">격차 5~10%p · 하락 추세</div>
        </div>
      </div>

      {/* 요약 텍스트 바 */}
      <div className="bg-white border border-slate-200 shadow-sm rounded-xl px-4 py-2.5 text-sm text-slate-600">
        이번달 LTV 점검 결과, 총{" "}
...
function SummaryPanel({ tone, title, desc, items, onItemClick }) {
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
      {/* 패널 헤더 */}
      <div className="bg-slate-50 px-4 py-1.5 flex items-center justify-between border-b border-slate-200">
      ...
      {/* 컬럼 헤더 */}
      <div className="grid grid-cols-3 px-4 py-1.5 border-b border-slate-100 bg-slate-50/50">
        <span className="text-[14px] font-bold text-slate-500">지역 / 용도</span>
        <span className="text-[14px] font-bold text-slate-500 text-center">현재</span>
        <span className="text-[14px] font-bold text-slate-500 text-center">권고안</span>
      </div>
          ...
          items.map((item, idx) => {
            ...
            return (
              <button
                ...
                className="w-full grid grid-cols-3 items-center px-4 py-3 hover:bg-slate-50 transition-all duration-150 border-b border-slate-100 last:border-0 text-left"
              >
                <div>
                  <div className="text-[17px] font-extrabold text-slate-900">{item.region} / {item.usage}</div>
                </div>
                <div className="text-[20px] font-black text-slate-700 text-center">
                  {item.current_ltv}%
                </div>
                <div className="text-center">
                  <span className={`text-[20px] font-black ${recoCls}`}>{reco != null ? `${reco}%` : "-"}</span>
                  {delta != null && (
                    <span className={`text-sm font-bold ml-1 ${deltaCls}`}>
                      ({delta > 0 ? "+" : ""}{delta}%p)
                    </span>
                  )}
                </div>
