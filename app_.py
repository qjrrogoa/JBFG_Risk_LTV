...
# 2. 대분류 선택 (그래프용)
selected_category = st.sidebar.radio("대분류 선택 (상세 분석용)", ["주택", "건물", "토지"])
...
# 4-1. 극단값 제외 기준 (number_input으로 변경)
outlier_input = st.sidebar.number_input("극단값 제외 기준 (%)", min_value=1.0, max_value=100.0, value=20.0, step=1.0)
outlier_threshold = outlier_input / 100.0

# 4-2. 비교 기준 (B) 개월 수 선택
b_months_options = [3, 6, 12, 36, 60]
b_months = st.sidebar.selectbox("비교 기준 (B) 개월 수", options=b_months_options, index=b_months_options.index(12))

# 4-3. Median(중간값) 기준 개월 수 선택
median_months_options = [3, 6, 12, 36, 60]
median_months = st.sidebar.selectbox("Median 기준 개월 수", options=median_months_options, index=median_months_options.index(12))
...
def calculate_metrics(df, target_usage, ltv, current_date, mode, outlier_thresh, b_m, med_m):
    sub_df = df[df['분석용도'] == target_usage]
    ...
    if mode == "월별 (최근)" or mode == "월별 (극단값 제외)":
        def get_avg_months(months):
            ...
            mask = (sub_df['매각일'] > start_date) & (sub_df['매각일'] <= current_date)
            period_df = sub_df.loc[mask]
            if period_df.empty: return None
            return period_df['낙찰율'].mean()
            
        def get_median_months(months):
            start_date = current_date - relativedelta(months=months)
            mask = (sub_df['매각일'] > start_date) & (sub_df['매각일'] <= current_date)
            period_df = sub_df.loc[mask]
            if period_df.empty: return None
            return period_df['낙찰율'].median()
        ...
        b_count = get_count_months(b_m)

        return get_avg_months(3), get_avg_months(6), get_avg_months(12), get_avg_months(36), get_avg_months(60), get_median_months(med_m), b_count
...
cols_labels.append(f"Median ({median_months}개월)")
...
for category, types in LTV_CONFIG.items():
    for usage_type, ltv in types.items():
        val1, val2, val3, avg_3y, avg_5y, val_med, b_count = calculate_metrics(
            winning_df, usage_type, ltv, last_date, analysis_mode, outlier_threshold, b_months, median_months
        )
        
        # val1~avg_5y 중 선택된 b_months와 일치하는 값을 찾아 판단 기준으로 삼음
        val_map = {3: val1, 6: val2, 12: val3, 36: avg_3y, 60: avg_5y}
        target_val = val_map.get(b_months, None)
        
        # 값이 없으면 "모수 부족"
        if target_val is None:
            judgment = "모수 부족"
            action_step = "⚪" # Gray/White
            gap = 0 # 의미 없음 (표시 안 함)
            current_rate = 0 # 의미 없음
        else:
            current_rate = target_val
            gap = current_rate - ltv
        ...
        row_data = {
            ...
            cols_labels[5]: val_med if val_med is not None else "-",
            "Gap (B-A)": gap if target_val is not None else "-",
...
# 상세 분석 선택 UI (HTML 테이블 복원 후 기능을 위해 위로 배치)
col_sel, col_btn = st.columns([3, 1])
with col_sel:
    # 현재 선택된 대분류에 해당하는 용도만 추출
    usage_list_for_dropdown = []
    for u_cat, u_types in LTV_CONFIG.items():
        if u_cat == selected_category:
            usage_list_for_dropdown.extend(list(u_types.keys()))
...
# Display DF 준비
# st.table이나 st.dataframe으로는 스타일과 병합을 동시에 완벽하게 제어하기 어려움.
# 엑셀 같은 '병합(Rowspan)'을 확실하게 보여주기 위해 HTML로 변환하여 렌더링.

# 기존 스타일링 로직을 적용하기 위해 값을 포맷팅한 데이터프레임 생성 (인덱스 제외)
formatted_df = display_df.copy()
formatted_df["LTV(A)"] = formatted_df["LTV(A)"].apply(lambda x: f"{x:.0f}%")
...
# 색상 적용 로직 (HTML 생성 시 적용) - 사용자 피드백 반영 (전체 다크 스타일 통일)
# 사용자가 "다크 레드/옐로우" 스타일을 선호하며, 초록/회색도 같은 스타일(다크 배경)로 요청함.
def get_color_style(val):
    if val == "조정 필요": # Dark Red background
        return 'background-color: #5a1e1e; color: #ffcccc'
    elif val == "조정 여부 검토": # Dark Yellow background
        return 'background-color: #5a5a1e; color: #ffffcc'
    elif val == "모수 부족": # Dark Gray background
        return 'background-color: #3e3e3e; color: #cccccc; font-style: italic;'
    elif val == "현행 유지": # Dark Green background
        return 'background-color: #1e4620; color: #ccffcc; font-weight: bold;'
    return ''

# Styler 객체 생성
styler = formatted_df.style.set_properties(**{'text-align': 'center', 'vertical-align': 'middle'})

# 헤더 스타일 (Streamlit 다크모드 헤더와 비슷하게)
styler.set_table_styles([
    {'selector': 'th', 'props': [
        ('text-align', 'center'), 
        ('vertical-align', 'middle'), 
        ('background-color', '#0e1117'),  # Streamlit 기본 배경색에 가까운 아주 어두운 색
        ('color', '#fafafa'), 
        ('font-weight', 'bold'),
